...
#define SSIZE  500000
	...
	c(!strcmp(p=stb_dupreplace("testfootffooo foo fox", "foo", "brap"), "testbraptfbrapo brap fox"), "stb_dupreplace 1");
	free(p);
	c(!strcmp(p=stb_dupreplace("testfootffooo foo fox", "foo", ""    ), "testtfo  fox"            ), "stb_dupreplace 2");
	free(p);
	c(!strcmp(p=stb_dupreplace("abacab", "a", "aba"),                   "abababacabab"            ), "stb_dupreplace 3");
	free(p);
	...
	if (!f)
	{
		stb_fatal("Error: couldn't open file just written\n");
	}
	else
	{
		char temp[4];
		...
		if (fread(temp,1,4,f) == 0)
		{
			...
			if (n)
			{
				stb_fatal("Error reading from stream: %d", n);
			}
	...
	stb_strncpy(buf, "foobar", 6);
	c(strcmp(buf,"fooba" )==0, "stb_strncpy() 1");
	stb_strncpy(buf, "foobar", 8);
	c(strcmp(buf,"foobar")==0, "stb_strncpy() 2");

	c(!strcmp(p=stb_duplower("FooBar"), "foobar"), "stb_duplower()");
	free(p);
	strcpy(buf, "FooBar");
	stb_tolower(buf);
