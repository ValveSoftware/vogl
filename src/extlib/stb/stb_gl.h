...
int stbgl_TexImage2D_Extra(int texid, int w, int h, void *data, int chan, char *props, int preserve_data)
{
	...
	while (*props)
	{
		...
		case 'w' :
			...
			break;
		case 'C' :
			if (wrap_s == STBGL_UNDEFINED) wrap_s = GL_CLAMP ;
			else wrap_t = GL_CLAMP ;
			break;
		case 'c' :
			if (wrap_s == STBGL_UNDEFINED) wrap_s = STBGL_CLAMP_TO_EDGE;
			else wrap_t = STBGL_CLAMP_TO_EDGE;
			break;
		case 'f' :
			input_type = GL_FLOAT;
			break;
